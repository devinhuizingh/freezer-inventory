			...
			var listRef = new Firebase("https://freezer-inventory.firebaseio.com/"+user);
				listRef.on("value", function(snapshot) {
					...
					$.each(data, function(key, value) {
						...
						if (difference > 365){
							$('#list').append('<li><input id="checkbox-'+i+'" class="checkbox-custom" type="checkbox" value="' + " " + value.item + '" >' + '<label for="checkbox-'+i+'" class="checkbox-custom-label">' + value.item + ' ' + value.date +  '</label><button  value="' + key + '"class="deletebutton">Remove</button></li> ');
			...
			if (items.length >= 1) {
				...
				$.get(url, function(data){
					...
					$.each(data.matches, function() {
					  ...
					  bigPic= (string).replace('=s90', '=s300');
					  //console.log(bigPic);
					  $('#recipe').prepend('<a href="http://www.yummly.com/recipe/' + this.id + '"><li class="listed"><img src="' + bigPic +  '" class="pic"><span>'+ this.recipeName +'</span> </li></a>');
					})
				})
